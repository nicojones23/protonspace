...
async function route(req: IncomingMessage, res: ServerResponse) {
  ...
  if (req.method === 'GET' && url.pathname === '/api/auth/session') { const current=session(req); if(!current)return json(res,200,{authenticated:false}); const [rows]=await pool.query('SELECT public_id AS publicId,username,display_name AS displayName FROM proton_accounts WHERE id=? AND account_status="active"',[current.accountId]); return json(res,200,{authenticated:true,account:(rows as any[])[0]??null}); }
  ...
  if (req.method === 'GET' && url.pathname === '/api/directory') { const [rows]=await pool.query('SELECT public_id AS publicId,username,display_name AS displayName,created_at AS joinedAt FROM proton_accounts WHERE account_status="active" ORDER BY last_login_at DESC,created_at DESC LIMIT 50'); return json(res,200,{people:rows}); }
